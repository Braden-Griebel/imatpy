...
def process_token(token, postfix, operator_stack, precedence):
    ...
    if token == ")":
        ...
        while len(operator_stack) > 0 and operator_stack[-1] != "(":
            op = operator_stack.pop()
            postfix.append(op)
        if len(operator_stack) == 0 or operator_stack[-1] != "(":  # Check for mismatch in parentheses
            raise ValueError("Mismatched Parenthesis in Expression")
...
def gene_to_rxn_weights(model: cobra.Model, gene_weights: pd.Series, fn_dict: dict = None,
                        fill_val: Any = 0) -> pd.Series:
    """
